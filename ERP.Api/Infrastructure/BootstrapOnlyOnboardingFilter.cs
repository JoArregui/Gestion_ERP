...
namespace ERP.Api.Infrastructure
{
    ...
    /// (ver empresas, crear la empresa y crear el primer usuario).
    /// Cualquier otro endpoint autenticado le responde 403.
    /// Las llamadas anónimas (login, onboarding-check, swagger) no se ven afectadas.
    /// </summary>
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            if (BootstrapUser.IsBootstrapUser(context.HttpContext.User))
            {
                ...
                var allowed =
                    ...
                    (method == "GET" && path == "/api/empresas/onboarding-check") ||
